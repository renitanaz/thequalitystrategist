...
| 4 | Search with empty box | Graceful empty state or 400 | Page errors (BUG-014 surfaces via UI) | FAIL |
...
| 2 | Check whose orders show | Only this user's orders | Other users' orders visible too | FAIL (BUG-013) |
...
**BUG-013 across the session.** Logging in as one user and opening order history reveals other people's purchases. As an isolated endpoint it read as a 200 with unexpected rows; as a flow, it is a privacy incident a real user would notice and screenshot.

**BUG-014 through the UI.** The empty-search 500 from Phase 5 surfaces here as a broken-looking page mid-journey, connecting the backend fault to what the user actually sees.
